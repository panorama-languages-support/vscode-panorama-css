...
export interface Settings {
	panocss: LanguageSettings;
	panoscss: LanguageSettings;
}
...
export function startServer(connection: Connection, runtime: RuntimeEnvironment) {
	...
	connection.onInitialize((params: InitializeParams): InitializeResult => {
		...
		languageServices.panocss = getCSSLanguageService({ fileSystemProvider: requestService, clientCapabilities: params.capabilities });
		languageServices.panoscss = getSCSSLanguageService({ fileSystemProvider: requestService, clientCapabilities: params.capabilities });
	...
	function getLanguageService(document: TextDocument) {
		...
		if (!service) {
			connection.console.log('Document type is ' + document.languageId + ', using panocss instead.');
			service = languageServices['panocss'];
		}
		return service;
	}
