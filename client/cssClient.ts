...
export function startClient(context: ExtensionContext, newLanguageClient: LanguageClientConstructor, runtime: Runtime) {
	...
	const customDataSource = getCustomDataSource(context.subscriptions, context.extensionUri);

	let documentSelector = ['panocss', 'panoscss'];

	// Options to control the language client
	let clientOptions: LanguageClientOptions = {
		documentSelector,
		synchronize: {
			configurationSection: ['panocss', 'panoscss']
		},
	...
	let client = newLanguageClient('panocss', 'Panorama CSS Language Server', clientOptions);
	...
	commands.registerCommand('_panocss.applyCodeAction', applyCodeAction);

	function applyCodeAction(uri: string, documentVersion: number, edits: TextEdit[]) {
		let textEditor = window.activeTextEditor;
		if (textEditor && textEditor.document.uri.toString() === uri) {
			if (textEditor.document.version !== documentVersion) {
				window.showInformationMessage(`Panorama CSS fix is outdated and can't be applied to the document.`);
			}
			textEditor.edit(mutator => {
				...
				if (!success) {
					window.showErrorMessage('Failed to apply Panorama CSS fix to the document. Please consider opening an issue with steps to reproduce.');
				}
			});
		}
	}
